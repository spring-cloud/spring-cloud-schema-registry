...
Spring Cloud Stream provides support for schema-based message converters through its `spring-cloud-schema-registry-client` module.
...
The `spring-cloud-schema-registry-client` module contains two types of message converters that can be used for Apache Avro serialization:
